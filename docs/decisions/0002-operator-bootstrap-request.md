...
that authorizes the first generation, `000000`, through a bootstrap Lambda.
...
retained logs, an error alarm, and an event source mapping. Only insertion of
`BOOTSTRAP/REQUEST` with status `REQUESTED` invokes the handler; changing
...
then inserts the request and an audit event without overwriting either record.
The request contains an operator identity, request ID, a 15-minute validity
window, and a digest of approved control values. The digest excludes only
...
first stack; and conditionally changes REQUESTED to CREATING. Immediately before
...
UNINITIALIZED, the request must remain valid, and HOLD must be absent.
...
after stack deletion. Cancellation before claim changes status to CANCELLED.
After claim, use HOLD and inspect; cancellation cannot undo an in-flight AWS call.
